        ...
        _move_node_internal: function (node, beforeid) {
            if (!!node && !!beforeid) {
                if (beforeid == '_last_') {
                    ...
                    node.index = 0;
                    this._reindex(node.parent);
                } else if(beforeid == '_up_'){
					var node_before = this.get_node_before(node);
				...
				else if(beforeid == '_down_'){
					var node_after = this.get_node_after(node);
					if(node_after != null && node_after.parent != null && node_after.parent.id == node.parent.id){
						node.index = node_after.index + 0.5;
						this._reindex(node.parent);
					}
				}
				else {
    ...
    jm.format = {
        ...
        node_array: {
            example: {
                ...
                "format": "node_array",
                "data": [
                    { "id": "root", "topic": "新建脑图", "isroot": true }
                ]
            },
            ...
            if (this.get_editable()) {
                ...
                if (!!node) {
                    ...
                    if (!!size) {
						...
						console.log("set_node_font_style",size);
                    }
                    if (!!weight) {
